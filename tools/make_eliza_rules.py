...
def render(greeting, subs, tags, keywords, memory, none):
    ...
    w("")
    w("**Filter by kind before reading payload.** Every template is a 3-tuple,")
    w("but payload is a string only for the reply kinds: NEWKEY carries None")
    w("and PRE carries a (rewrite, keyword) pair. A loop that unpacks three")
    w("values and uses the third gets an arity that looks right and a value")
    w("that is not text.")
    w("")
    w("MEMORY and NONE are separate module-level tuples, not part of RULES. A")
    w("sweep that walks RULES alone silently misses them, which has already")
    w("produced two different answers to the same measurement twice.")
    w('"""')
    w("")
    w('CANNED = "C"')
    w('LITERAL = "L"')
